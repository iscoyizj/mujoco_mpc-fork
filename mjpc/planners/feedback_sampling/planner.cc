...
void FeedbackSamplingPlanner::UpdateNominalPolicy(int horizon, ThreadPool& pool) {
  // We assume that iLQG is used to compute the nominal feedback policy.
  // Set the iLQG solver state and reset if needed. We have already done so in Reset/SetState.
  // Just call ilqg_solver.OptimizePolicy() to get the best policy from iLQG.
...
int FeedbackSamplingPlanner::OptimizePolicyCandidates(int ncandidates, int horizon,
  ...
  // We already updated the nominal policy in OptimizePolicy or can be done here as needed.
  // But since it's already done in OptimizePolicy, we can skip here if desired.
  // UpdateNominalPolicy(horizon, pool);
...
void FeedbackSamplingPlanner::AddNoiseToPolicy(int i) {
  ...
  // Add noise to feedforward actions
  for (int t = 0; t < horizon - 1; t++) {
    for (int u = 0; u < nu; u++) {
      double scale = 0.5*(model->actuator_ctrlrange[2*u+1] - model->actuator_ctrlrange[2*u]);
      double noise_val = absl::Gaussian<double>(gen_, 0.0, scale * std_dev);
      cp.trajectory.actions[t*nu + u] += noise_val;
    }
    Clamp(&cp.trajectory.actions[t*nu], model->actuator_ctrlrange, nu);
  }

  // Optional: add noise to feedback gains if desired
  // int dim_state_derivative = 2 * model->nv + model->na;
  // for (int t = 0; t < horizon - 1; t++) {
  //   for (int idx = 0; idx < nu * dim_state_derivative; idx++) {
  //     double noise_val = absl::Gaussian<double>(gen_, 0.0, std_dev * 0.1);
  //     cp.feedback_gain[t*nu*dim_state_derivative + idx] += noise_val;
  //   }
  // }
